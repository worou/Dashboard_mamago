...
export default function Pays() {
  ...
  const submit = async () => {
    ...
    setBusy(true);
    try {
      await api.createPays({ nom_pays: form.nom_pays, code_iso: form.code_iso.toUpperCase().slice(0, 2), devise: form.devise || 'XOF' });
      toast(form.nom_pays + ' ajouté');
  ...
  return (
    <div style={{ animation: 'floatIn .35s ease both' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap', marginBottom: 18 }}>
        ...
        <button onClick={() => { setForm({ devise: 'XOF' }); setModal(true); }} style={styleObj(S.btnGreen)}>+ Ajouter un pays</button>
      </div>
      ...
      {list.length === 0 ? (
        ...
        <div className="mg-cards3" style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 16 }}>
          {list.map((p) => {
            ...
            return (
              <div key={p.id} onClick={() => nav('/stats/' + p.id)} style={{ ...styleObj(S.cardPad('18px')), cursor: 'pointer' }}>
